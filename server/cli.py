    ...
    def _validate_keybinds(self) -> dict[str, Any]:
        ...
        if test_player:
            for key, keybinds in self.game._keybinds.items():
                ...
                event: dict[str, Any] = {"type": "keybind", "key": key}
...
def cmd_show_options(args):
    ...
    for field_name in options_obj.__dataclass_fields__:
        ...
        meta = get_option_meta(type(options_obj), field_name)
        if meta:
...
def cmd_simulate(args):
    ...
    if args.json:
        ...
        if total_sounds == 0:
            ...
            print(
                f"\nSound events: {total_sounds} total "
                f"({sound_info.get('spectator', 0)} broadcast, "
                f"{sound_info.get('players', 0)} per-player)"
            )
        ...
        if kb:
            if kb["passed"]:
                print(
                    f"\nKeybind validation: passed ({len(kb['keybinds'])} keybinds, "
                    f"{len(kb['smoke_tests'])} smoke tests)"
                )
            else:
...
def main():
    ...
    sim_parser.add_argument(
        ...
        "-l",
        default="en",
        help="Locale for output (e.g., vi). Non-English exposes hardcoded strings.",
    )
    sim_parser.add_argument(
        "--validate-keybinds",
        "-k",
